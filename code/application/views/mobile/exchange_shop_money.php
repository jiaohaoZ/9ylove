...
<header class="mui-bar mui-bar-nav nav-bg" style="box-shadow: none;">
    ...
    <h1 class="mui-title">兑换购物积分</h1>
</header>
<div class="mui-content">
    <div>
        <p style="text-align: center; line-height: 1rem; font-size: 0.28rem;">当前可用爱心积分为<?=$gold_money?>个</p>
        <form style="margin-top: 0.3rem;" action="" method="post" id="myForm">
            <div class="mui-input-row mui-input-group">
                <label>兑换积分数</label>
                <input class="mui-input-clear" name="num" id="num" type="text" placeholder="请输入购物积分数量">
            </div>
            <div style="margin: 0.5rem;">
                <button type="button" disabled id="btnConfirm" class="mui-btn my-btn-block-green-1">确认兑换</button>
            </div>
        </form>
    </div>
</div>
        ...
        document.getElementById('btnConfirm').addEventListener('click', function () {
            ...
            mui.prompt('请输入支付密码', '请输入6位密码', '向日葵爱心', btnArray, function (e) {
                if (e.index == 1) {
                    $.ajax({
                        type: "post",
                        url: "<?=site_url('money/exchangeShopMoney')?>",
                        data: {
                            num : num.value,
